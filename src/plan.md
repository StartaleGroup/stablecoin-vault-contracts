# Two-vault yield system (USDSC asset) [ DRAFT ]
...
- sUSDSCVault (checkbox ON): ERC-4626 vault (asset=USDSC); users deposit USDSC -> mint sUSDSC; external transfers to the vault increase totalAssets without minting shares -> PPS rises.
- RewardRedistributor (yieldRecipient of USDSC MYieldToOne): on distribute():
  1) minted = USDSC.claimYield()
  2) fee = minted*10% -> Startale; net = minted - fee
  3) read TVLs: Tclaim = ClaimVault.totalPrincipal(); T4626 = sUSDSCVault.totalAssets(); T = Tclaim + T4626
  ...
  5) transfer toClaim to ClaimVault and call onYield(toClaim)
     transfer to4626 to sUSDSCVault (PPS rises)

Key properties:
- Pro-rata split across cohorts by TVL with rounding-carry (long-run exactness).
- sUSDSC price (PPS) = totalAssets/totalSupply; on-chain source of truth; AMMs arbitrage to it.
...
- vaults/4626/SUSDSCVault.sol  (ERC-4626; roles: PAUSER_ROLE)
- distributor/RewardRedistributor.sol (roles: OPERATOR_ROLE; DEFAULT_ADMIN for params)
- interfaces/IClaimVault.sol, interfaces/IUSDSCMExtension.sol

Deployment:
- Set USDSC extension yieldRecipient = RewardRedistributor
...
- sUSDSCVault: totalAssets increases exactly by transfers; PPS non-decreasing barring withdrawals/loss
