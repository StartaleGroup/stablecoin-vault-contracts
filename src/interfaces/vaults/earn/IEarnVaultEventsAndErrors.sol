...
interface IEarnVaultEventsAndErrors {
  ...
  event YieldRedistributorChanged(
    address indexed actor, address indexed oldRedistributor, address indexed newRedistributor
  );
  ...
  event BlacklistStatusChanged(address indexed actor, address indexed user, bool oldStatus, bool newStatus);
  ...
  event BoostRewardIndexed(
    address indexed token, uint256 amount, uint256 newBoostGlobalIndex, uint256 newBoostClaimReserve
  );
