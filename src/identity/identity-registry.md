...
**Current (registry not deployed):** nothing in this section applies — `EarnVaultV2` credits addresses directly and never reads this contract. The V2 upgrade procedure and the engine handoff (event ABI, cycle rules, per-batch cap) live with `EarnVaultV2`: see its NatSpec and `src/vaults/earn/base-tier-auto-compounding.md`.
