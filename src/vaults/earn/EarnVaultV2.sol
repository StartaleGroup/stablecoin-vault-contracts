...
contract EarnVaultV2 is EarnVaultUpgradeable {
  ...
  function _creditBoostEntry(
    ...
    // guard against crediting principal to itself (IdentityRegistry does not track the vault).
    // Users can't bind to it - switchAddress() needs a signature from newAddr, which the vault
    // can't produce - but a backend-signed register()/registerBatch() could by mistake. An address
    // can become blacklisted any time after binding. Either way the entry is skipped, not credited.
    EarnVaultStorage storage $ = _getStorage();
