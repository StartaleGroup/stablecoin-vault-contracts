...
- **Storage/initialization, as originally shipped vs. now.** At the time this was written, `EarnVaultV2`'s constructor just called `_disableInitializers()` and upgrading a live V1 proxy to it needed only an empty-calldata `upgradeAndCall`. That's no longer true: a later change (the `onBoostCredit()` VIP-boost work, alongside `src/identity/IdentityRegistry.sol`) added a boostKeeper role and a settable `IIdentityRegistry` reference directly to this same `EarnVaultV2.sol`, in its own ERC-7201 namespace. Upgrading a live V1 proxy to the current `EarnVaultV2` now requires calling `initializeV2(initialBoostKeeper, initialIdentityRegistry)` via `reinitializer(2)`, bundled into the same `ProxyAdmin.upgradeAndCall` transaction as the upgrade itself. `initializeV2` enforces this on-chain: it only accepts the proxy's ERC-1967 admin (the ProxyAdmin) as caller, which on a Transparent proxy can only reach it through `upgradeAndCall` - see that function's NatSpec.
...
**Execution-mode correction (2026-09-08):** the numbers below are measured under `forge test --isolate` (equivalently, `--gas-report`, which is what CI's `make gas-report` runs and what caught this). Plain `forge test` (no isolation) executes an entire test function — including all of `_seedUsers`' setup activity — as one shared EIP-2929 warm/cold-access context, which makes a second `compoundMany()` call *within the same test function* look artificially cheap (it inherits "warm" storage access from the first call and from setup). That's not how production works: a keeper's "day 2" `compoundMany()` is a genuinely separate on-chain transaction, and warm/cold access state never persists across transactions — every real transaction pays cold-access costs again, regardless of what an earlier transaction touched. `--isolate` mode correctly models this, so its (higher) numbers are the ones to trust:

| Scenario | Measured marginal gas/user (`--isolate`) | Safe batch size | Batches for 31,799 wallets |
|---|---|---|---|
| No active boost tokens | 16,316 | ~1,438 | 23 |
| 2 active boost tokens, steady state | 41,380 | ~574 | 56 |
| 2 active boost tokens, first-ever touch (cold, one-time only) | 109,810 | ~217 | 147 (once) |

(For reference, non-isolated `forge test` reports optimistic-but-inaccurate figures of ~4,551 / ~10,415 / ~98,043 gas/user for the same three rows respectively — these were what an earlier version of this doc cited before the discrepancy was caught by a CI gas-report run failing an assertion sized to the wrong numbers.)
