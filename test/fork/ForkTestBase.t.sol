...
contract ForkTestBase is TaskBase, ConfigureBase, DeployBase, Test {
  ...
  function setUp() public virtual {
    ...
    WormholeTransceiverConfig memory hubTransceiverConfig_ =
      WormholeConfig.getWormholeTransceiverConfig(ethereumChainId_);

    _hubWormholeCore = hubTransceiverConfig_.coreBridge;
    _hubGuardian = new WormholeSimulator(_hubWormholeCore, _DEVNET_GUARDIAN_PK);

    (_hubPortal, _hubWormholeTransceiver) =
      _deployHubComponents(_DEPLOYER, ethereumWormholeChainId_, hubDeployConfig_, hubTransceiverConfig_);
    ...
    WormholeTransceiverConfig memory arbitrumSpokeTransceiverConfig_ =
      WormholeConfig.getWormholeTransceiverConfig(arbitrumChainId_);
    ...
    (_arbitrumSpokePortal, _arbitrumSpokeWormholeTransceiver, _arbitrumSpokeRegistrar, _arbitrumSpokeMToken) =
    ...
    (_arbitrumSpokeWrappedMTokenImplementation, _arbitrumSpokeWrappedMTokenProxy) = _deploySpokeWrappedMToken(
      _DEPLOYER, _arbitrumSpokeMToken, _arbitrumSpokeRegistrar, _arbitrumSpokeVault, _MIGRATION_ADMIN, _burnNonces
    );
    ...
    WormholeTransceiverConfig memory optimismSpokeTransceiverConfig_ =
      WormholeConfig.getWormholeTransceiverConfig(optimismChainId_);
    ...
    (_optimismSpokePortal, _optimismSpokeWormholeTransceiver, _optimismSpokeRegistrar, _optimismSpokeMToken) =
    ...
    (_optimismSpokeWrappedMTokenImplementation, _optimismSpokeWrappedMTokenProxy) = _deploySpokeWrappedMToken(
      _DEPLOYER, _optimismSpokeMToken, _optimismSpokeRegistrar, _optimismSpokeVault, _MIGRATION_ADMIN, _burnNonces
    );
  ...
  function _deliverMessage(address wormholeRelayer_, bytes memory signedMessage_) internal {
    ...
    IWormholeRelayer(wormholeRelayer_).deliver{value: 1 ether}(
      new bytes[](0), signedMessage_, payable(address(this)), new bytes(0)
    );
  }
  ...
  function _propagateMIndex(uint16 spokeChainId_, uint256 spokeForkId_, address spokeRelayer_) internal {
    ...
    _sendMTokenIndex(
      _hubPortal, spokeChainId_, address(this).toBytes32(), _quoteDeliveryPrice(_hubPortal, spokeChainId_)
    );
