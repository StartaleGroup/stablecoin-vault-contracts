...
contract RewardRedistributorIntegrationTest is Test {
  ...
  function testIntegration_ConservationWithRealVaults() public {
    ...
    uint256 distributed = (finalStartaleBalance - initialStartaleBalance) + (finalEarnBalance - initialEarnBalance)
  ...
  function testIntegration_EarnVaultWithdrawalsAfterDistribution() public {
    ...
    assertEq(
      charlieNewPrincipal,
      charlieInitialPrincipal - partialWithdrawAmount,
      'Charlie principal reduced by withdrawal amount'
    );
    assertEq(
      charlieBalanceAfter - charlieBalanceBefore,
      partialWithdrawAmount + charlieClaimableBefore,
      'Charlie received principal + all accrued yield'
    );
    ...
    assertEq(
      charlieBalanceAfterFullWithdraw - charlieBalanceBeforeFullWithdraw,
      charlieRemainingPrincipal + charlieNewClaimable,
      'Charlie received remaining principal + yield'
    );
    vm.stopPrank();
  }
  ...
  function testIntegration_SUSDSCVaultRedemptionsAfterDistribution() public {
    ...
    uint256 initialPPS =
      susdscVault.totalSupply() > 0 ? (susdscVault.totalAssets() * 1e18) / susdscVault.totalSupply() : 1e18;
  ...
  function testIntegration_MixedWithdrawalsAfterMultipleDistributions() public {
    ...
    assertEq(
      ...
      alicePartialWithdraw + aliceSecondClaimable,
      'Alice received partial principal + all accrued yield'
    );
    assertEq(
      earnVault.principal(alice), aliceInitialEarnPrincipal - alicePartialWithdraw, 'Alice principal reduced correctly'
    );
  ...
  function testIntegration_WithdrawalOrderingAndInvariants() public {
    ...
    assertEq(
      totalEarnPrincipalAfter,
      totalEarnPrincipalBefore - aliceWithdrawAmount,
  ...
  function testIntegration_CarryMechanismAccuracy() public {
    ...
    uint256 yieldLoss = totalYieldToEarnVault > totalYieldClaimed ? totalYieldToEarnVault - totalYieldClaimed : 0;

    // Loss should be very small relative to total yield
    if (totalYieldToEarnVault > 0) {
      uint256 lossPercentage = (yieldLoss * 10_000) / totalYieldToEarnVault; // basis points
  ...
  function testIntegration_BoostRewardsWithWithdrawals() public {
    ...
    assertEq(
      aliceUsrdBalanceAfter - aliceUsrdBalanceBefore,
      withdrawAmount + aliceClaimableUsrd,
      'Alice received principal + USDSC yield'
    );
    assertEq(
      aliceAstrBalanceAfter - aliceAstrBalanceBefore, aliceClaimableAstr, 'Alice received boost rewards on withdrawal'
    );
  ...
  function testIntegration_RewardRedistributorPreviewFunctions() public {
    ...
    assertEq(minted, 5000e6, 'Preview shows correct minted amount');
    ...
    (
      ...
      uint256 previewEarn,
      uint256 previewOn,,
      uint256 previewSBase,
      uint256 previewTEarn,
      uint256 previewTYield
    ) = rr.previewSplit(5000e6);
  ...
  function testIntegration_EarnVaultSurplusSweep() public {
    ...
    usdsc.mint(address(earnVault), 5000e6);
  ...
  function testIntegration_EarnVaultRoleManagement() public {
    ...
    usdsc.mint(address(earnVault), 1000e6);
    vm.prank(newYieldRedistributor);
    earnVault.onYield(1000e6); // Should work
  }
  ...
  function testIntegration_EarnVaultViewFunctionEdgeCases() public {
    ...
    (uint256 userPrincipal, uint256 userClaimable, uint256 userTotal, uint256 userLastIndex) =
      earnVault.getUserInfo(newUser);
