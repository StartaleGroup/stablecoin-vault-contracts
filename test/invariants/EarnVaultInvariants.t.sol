...
contract EarnVaultInvariants is Test {
  ...
  function test_USDSCBalanceInvariant() public {
    ...
    assertGe(
      usdsc.balanceOf(address(vault)), vault.claimReserve(), 'USDSC balance should be >= claimReserve after yield'
    );
    ...
    assertGe(
      usdsc.balanceOf(address(vault)), vault.claimReserve(), 'USDSC balance should be >= claimReserve after withdrawal'
    );
  }
  ...
  function test_PrincipalSumInvariant() public {
    ...
    assertEq(
      alicePrincipal + bobPrincipal + charliePrincipal,
      totalPrincipal,
      'Sum of user principals should equal totalPrincipal'
    );
    ...
    assertEq(
      vault.principal(alice) + vault.principal(bob) + vault.principal(charlie),
      vault.totalPrincipal(),
      'Sum should equal totalPrincipal after yield'
    );
    ...
    assertEq(
      vault.principal(alice) + vault.principal(bob) + vault.principal(charlie),
      vault.totalPrincipal(),
      'Sum should equal totalPrincipal after withdrawal'
    );
  }
  ...
  function test_BoostInvariant() public {
    ...
    assertGe(
      boostToken.balanceOf(address(vault)),
      vault.boostClaimReserve(address(boostToken)),
      'Boost token balance should be >= boostClaimReserve'
    );
